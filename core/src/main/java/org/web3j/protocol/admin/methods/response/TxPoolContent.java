...
public final class TxPoolContent extends Response<TxPoolContent.TxPoolContentResult> {
    public static class TxPoolContentResult {
        ...
        public List<Transaction> getPendingTransactions() {
            return pending.values().stream().map(Map::values).flatMap(Collection::stream).collect(Collectors.toList());
        }

        public List<Transaction> getQueuedTransactions() {
            return queued.values().stream().map(Map::values).flatMap(Collection::stream).collect(Collectors.toList());
        }
